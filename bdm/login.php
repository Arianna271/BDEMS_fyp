...
if($act == "login")
{	
	$SQL_login 	= " SELECT * FROM `donor` WHERE `username` = '$username' AND `passwordDonor` = '$passwordDonor' ";
	...
	if($valid > 0)
	{
		...
		$_SESSION["passwordDonor"] 	= $data["passwordDonor"];
		$_SESSION["donorID"] 		= $data["donorID"];
		...
		<div class="w3-content w3-container w3-large" style="max-width:800px">			
			...
			<form action="" method="post" class="">
				...
				<div class="w3-section">
					...
					<input class="w3-input w3-border w3-round-large cpwdx" type="password" name="passwordDonor" id="passwordDonor" placeholder="" required>
					<!--username: <b>demo</b>  &nbsp;&nbsp;&nbsp; password: <b>P@ssw0rd</b>-->
					<div class="w3-padding "><input type="checkbox" onclick="myFunction()"> Show Password</div>
				</div>
			  
				<script>
				function myFunction() {
				  var x = document.getElementById("passwordDonor");
				  if (x.type === "password") {
					x.type = "text";
				  } else {
					x.type = "password";
			...
			<div class="w3-center">
			Don't have an account? <a href="register.php" class="w3-text-maroon">Register</a>
			</div>
		</div>
